...
async function main() {
  const seed: [string,string,number][] = JSON.parse(readFileSync('./src/lib/dict_words_seed.json', 'utf8'));
  ...
  for (let i = 0; i < seed.length; i += BATCH) {
    ...
    try {
      const res = await pool.query(`INSERT INTO dict_words (word, chosung, word_len) VALUES ${vals} ON CONFLICT DO NOTHING`, params);
      inserted += res.rowCount || 0;
    } catch(e: any) { console.error(`배치 ${i} 오류:`, e.message); }
